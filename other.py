...
app_ui = ui.page_navbar(
  ...
  sidebar = ui.sidebar(
    ...
    # ui.hr(),
    ui.input_action_button('plot_param', 'Plot parameters'),
    ui.hr(),
...
def server(input: Inputs, output: Outputs, session: Session):
  ...
  def compile_stan():
      try:
          stan_code = build_stan_program()
          stan_dir = Path(here("stan_files"))
          stan_dir.mkdir(parents=True, exist_ok=True)
          stan_file = stan_dir / f"{input.type_model()}_{input.attr_num()}_attr_model.stan"
          ...
          raise SafeException(f'Stan compilation failed: {e}')
  
  @render.text
  def compile_status():
    model = compiled_model.get()
    stan_file = compiled_stan_path.get()
    
    if model is None or stan_file is None:
      return 'Model not compiled yet.'
    
    return f'Compiled Stan model: {stan_file.name}'
  ...
  def run_model():
    ...
    if df is None or q is None:
      raise SafeException("Load the response data and Q-matrix first.")
